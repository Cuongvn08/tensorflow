...
def input_fn(df):
    ...
    for k in CONTINUOUS_COLUMNS:
        print('df[k].name={0}; dk[k].values={1}'.format(df[k].name, df[k].values))
    ...
    for k in CATEGORICAL_COLUMNS:
        print('df[k].name={0}; df[k].size={1}; dk[k].values={2}'.format(df[k].name, df[k].size, df[k].values))
...
def train_and_eval(model_dir, model_type, train_steps, train_data, test_data):
    ...
    # predict
    pred = list(m.predict(input_fn=lambda: input_fn(df_test)))
    print(pred)
